...
import unittest
from tethne.readers.coinfluence import read, _infer_spaces, CoInfluenceParser
...
datapath = '/Users/dwinter/git/tethne/tethne/tests/data/confluence/coinfluence.rdf'
datapath2 = '/Users/dwinter/git/tethne/tethne/tests/data/confluence/coinfluence2.rdf'
datapath3 = '/Users/dwinter/git/tethne/tethne/tests/data/zotero_withfiles/zotero_withfiles.rdf'
duplicatePath = '/Users/dwinter/git/tethne/tethne/tests/data/Duplicate'
...
class TestCoinfluenceDuplicates(unittest.TestCase):
...
class TestConinfluenceParser(unittest.TestCase):
    def test_read(self):
    ...
    def test_handle_date(self):
        parser = CoInfluenceParser(datapath)
        parser.parse()
    ...
    def test_parse(self):
        ...
        N = len(parser.data)
        self.assertEqual(N, 32, 'Expected 12 entries, found {0}.'.format(N))

    def test_coAuthnetwork(self):
        corpus = read(datapath, follow_links=True)
        self.assertIsInstance(corpus, Corpus)
        
        import tethne.networks as nt
        BC = nt.coauthors(corpus)
        assert len(BC.nodes()) > 0
        
    def test_coCitnetwork(self):
        corpus = read([datapath,datapath2], follow_links=True)
        self.assertIsInstance(corpus, Corpus)
        #print(len(corpus.citations))
       
        import tethne.networks as nt
        BC = nt.cocitation(corpus)
