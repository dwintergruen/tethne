...
SR = "http://ontologies.mpiwg-berlin.mpg.de/scholarlyRelations/"
CRM = "http://erlangen-crm.org/160714/"
...
class CoInfluenceParser(RDFParser):
    """
    Reads Coinfluence RDF files.
    """

    entry_class = Paper
    entry_elements = [ "<"+ SR + 'Commission>' ]
    tags = {
        'isPartOf': 'journal'
    }
    ...
    meta_elements = [
        ...
        ('authors_full', rdflib.URIRef(SR+"memberContainer")),
        ...
        ('citedReferences',rdflib.URIRef(SR+"referencedCommission")),
        ('documentType',
         rdflib.URIRef("http://www.zotero.org/namespaces/export#itemType"))]

    def __init__(self, path, **kwargs):
        if not isinstance(path, list) and  os.path.isdir(path):    # Preserve the old behavior.
            name = os.path.split(path)[1]
            path = os.path.join(path, '{0}.rdf'.format(name))

        super(CoInfluenceParser, self).__init__(path, **kwargs)
...
#             f.write(corrected)
        ...
        super(CoInfluenceParser, self).open()
    ...
    def handle_authors_full(self, value):
        ...
        member= rdflib.URIRef("http://ontologies.mpiwg-berlin.mpg.de/scholarlyRelations/member")
        
        authors = [self.handle_author(o) for s, p, o
                   in self.graph.triples((value, member, None))]
        return [a for a in authors if a is not None]
    
    def handle_citedReferences(self, value):
        
        comms= rdflib.URIRef("http://ontologies.mpiwg-berlin.mpg.de/scholarlyRelations/referencedCommission")
        
        comms = [self.handle_commission(o) for s, p, o
                   in self.graph.triples((value, comms, None))]
        return [a for a in comms if a is not None]
    ...
    def handle_commission(self, value):
        #member= rdflib.URIRef("http://ontologies.mpiwg-berlin.mpg.de/scholarlyRelations/member")
        
        #member_iter = self.graph.triples((value, member, None))
        title_iter = self.graph.triples((value,TITLE, None))
        
        
        try:
            title = [e[2] for e in title_iter][0]
        except IndexError:
            title = ''
        citation = self.entry_class()
        setattr(citation,"title",title)
        return citation
...
def read(path, corpus=True, index_by='uri', follow_links=False, **kwargs):
    ...
    parser = CoInfluenceParser(path, index_by=index_by, follow_links=follow_links)
    papers = parser.parse()
